...
function createNumberFormField(
  inputId,
  inputName,
  inputMin,
  inputValue,
...
function createRadioButtonField(
  inputId,
  inputName,
  inputValue,
  labelInnerHTML,
...
function createForm() {
  ...
  let titleFormField = createTextFormField(
    "book-title",
    "book-title",
    "Title of the book:",
  );
  let authorFormField = createTextFormField(
    "book-author",
    "book-author",
    "Author of the book:",
  );
  let numberOfPagesFormField = createNumberFormField(
  ...
  let alreadyReadRadioField = createRadioButtonField(
    "already-read",
    "book-read-or-not",
    "Book read",
    "Book read:",
  );
  let notReadYetField = createRadioButtonField(
    "not-read-yet",
    "book-read-or-not",
    "Not read yet",
    "Not read yet:",
  ...
  form.append(
    titleFormField,
    authorFormField,
    numberOfPagesFormField,
    alreadyReadRadioField,
    notReadYetField,
    formButtonsField,
  );

  const bookInputField = titleFormField.querySelector("input");
  bookInputField.addEventListener("input", () => {
    bookInputField.setCustomValidity("");
    bookInputField.reportValidity();
  });

  const authorInputField = authorFormField.querySelector("input");
  authorInputField.addEventListener("input", () => {
    authorInputField.setCustomValidity("");
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (bookInputField.value.trimEnd() === "") {
    ...
    if (authorInputField.value.trim() === "") {
      event.preventDefault();
      authorInputField.setCustomValidity(
    ...
    let bookNumberOfPages = document.getElementById(
      "book-number-of-pages",
    ).value;
...
class Book {
  ...
  showInfo() {
    console.log(
      `${this.title} by ${this.author}, ${this.numberOfPages} pages, ${this.haveRead ? "already read." : "not read yet!"}`,
    );
  }
}
